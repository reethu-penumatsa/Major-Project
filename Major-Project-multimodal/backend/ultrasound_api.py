...
# TensorFlow is optional at runtime; if it's missing, the ultrasound
# endpoints will return a 503 service-unavailable error so the rest
# of the app can run without forcing a specific Python/TensorFlow setup.
try:
...
if TF_AVAILABLE:
    ...
    print("Warning: TensorFlow not available — ultrasound endpoints disabled.")
    grad_model = None
...
def ultrasound_check():
    if not TF_AVAILABLE or grad_model is None:
        return (
            jsonify({"error": "Ultrasound functionality unavailable: TensorFlow not installed"}),
            503,
        )
